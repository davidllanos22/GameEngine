/*! GameEngine 2015-01-21 */
Camera = function(a, b) {
    this.game = a, this.name = b, this.x = 0, this.y = 0, this.width = a.width, this.height = a.height, 
    this.angle = 0;
}, Camera.prototype.setPosition = function(a, b, c) {
    c ? (this.x += .01 * (a - this.x), this.y += .01 * (b - this.y)) : (this.x = a, 
    this.y = b);
}, Camera.prototype.setRotation = function(a) {
    this.angle = a;
}, Entity = function(a, b, c) {
    this.x = a, this.y = b, this.name = c, this.parent, this.sprite, this.rect, this.childs = new Array(), 
    this.game, this.init();
}, Entity.prototype = {
    init: function() {},
    ...
    this.showPauseWhenNotFocused = !1, this.fillScreen = !1, this.meter = new FPSMeter({
        position: "absolute",
        width: 100,
        theme: "transparent"
    }), this.meter.hide(), this.showFps = !1, this.gameScale = 1, this.scale = 1, this.fillScreenWithRatio = !0, 
    this.ratio = 0, this.pixelart = !0;
    var c = this;
    ...
    this.cvs.style.height = this.height, this.initInternal();
}, Game.prototype = {
    initInternal: function() {
        Utils.log("init"), this.initDone = !1, this.loader = new Loader(), this.renderer = new Renderer(this), 
        this.input = new Input(this), this.timerManager = new TimerManager(this), this.pixi && this.pixiStage.addChild(this.renderer.graphics), 
        this.currentScene = new Scene(this, "Scene 01"), this.currentCamera = new Camera(this, "Camera 01"), 
        Utils.log("running"), this.fps = 60, this.dt = 0, this.start = new Date().getTime(), 
        this.step = 10 / this.fps, this.loop(this);
    },
    loop: function(a) {
    ...
    renderInternal: function() {
        ...
        this.ctx.msImageSmoothingEnabled = !0), this.renderer.clearScreen(), this.ctx.save(), 
        this.renderer.renderCounter = 0, this.ctx.translate(-this.currentCamera.x, -this.currentCamera.y), 
        this.render(), this.currentScene.renderInternal(), this.ctx.restore(), this.showPauseWhenNotFocused && !this.focused && (this.renderer.drawRect(0, 0, this.width, this.height, "rgba(0,0,0,0.8)"), 
        this.renderer.drawString("- PAUSED - ", this.width / 2 - 40, this.height / 2 - 20, 20, "white")), 
    ...
    setSize: function(a, b) {
        (0 == a || 0 == b) && Utils.logErr("Width and Height can't be 0."), this.width = a, 
        this.height = b, this.cvs.width = this.width, this.cvs.height = this.height;
    }
}, Keys = {
    NUM_0: 48,
    NUM_1: 49,
    ...
    this.mouseX = 0, this.mouseY = 0, this.mouseClick = [ !1, !1, !1 ], this.mouseRelease = [ !1, !1, !1 ], 
    ...
    onMouseMove: function(a, b) {
        ...
        this.mouseX = Math.round((b.clientX - c.left) / (c.right - c.left) * this.game.cvs.width / this.game.scale), 
        this.mouseY = Math.round((b.clientY - c.top) / (c.bottom - c.top) * this.game.cvs.height / this.game.scale);
    },
    ...
    this.game = a, this.pixi = this.game.pixi, this.ctx = this.game.ctx, this.renderCounter = 0, 
    ...
    drawImageOld: function(a, b, c, d, e, f) {
        this.ctx.drawImage(a, d * f, e * f, f, f, b, c, f, f), this.renderCounter++;
    },
...
TransitionScene.prototype.render = function() {
    this.visible.renderInternal(), this.game.renderer.clearScreen(this.fadeIn.isRunning ? "rgba(255,255,255," + this.fadeIn.count / this.time + ")" : "rgba(255,255,255," + (this.time - this.fadeOut.count) / this.time + ")");
